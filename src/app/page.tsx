...
export default function Home() {
  ...
  async function fetchToday() {
    ...
    if (!error) setRows((data as TodayRow[]) ?? []);
  }
  useEffect(() => { fetchToday(); }, [userId, todayStr]);
  ...
  if (!userId) {
    return (
      <div className="min-h-screen grid place-items-center p-6">
        <a href="/login" className="underline">Login dulu →</a>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="text-sm underline" onClick={() => supabase.auth.signOut().then(()=>location.reload())}>
          Keluar
        </button>
      </div>
      ...
      <div className="rounded-xl border p-4 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          <div>Tanggal: <b>{todayStr}</b></div>
          <div className="flex items-center gap-2">
            <label className="font-medium">Shift:</label>
            <label className="flex items-center gap-1">
              <input type="radio" name="shift" checked={selectedShift===1} onChange={()=>setSelectedShift(1)} />
              <span>1 (08:00–12:00)</span>
            </label>
            <label className="flex items-center gap-1">
              <input type="radio" name="shift" checked={selectedShift===2} onChange={()=>setSelectedShift(2)} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-1">
            <div>Clock-in: <b>{current?.clock_in_at ? format(new Date(current.clock_in_at), 'HH:mm') : '-'}</b></div>
            <div>Clock-out: <b>{current?.clock_out_at ? format(new Date(current.clock_out_at), 'HH:mm') : '-'}</b></div>
          </div>
          <div className="space-y-1">
            <div>Catatan: <b>{current?.notes || '-'}</b></div>
            <div>Foto:
              {current?.photo_url
                ? <a className="underline ml-1" href={current.photo_url} target="_blank">Lihat</a>
      ...
      <div className="rounded-xl border p-4">
        ...
        <ul className="text-sm list-disc ml-5 space-y-1">
          <li>Shift 1: {s1?.clock_in_at ? `In ${format(new Date(s1.clock_in_at),'HH:mm')}` : '-'} / {s1?.clock_out_at ? `Out ${format(new Date(s1.clock_out_at),'HH:mm')}` : '-'}</li>
          <li>Shift 2: {s2?.clock_in_at ? `In ${format(new Date(s2.clock_in_at),'HH:mm')}` : '-'} / {s2?.clock_out_at ? `Out ${format(new Date(s2.clock_out_at),'HH:mm')}` : '-'}</li>
        </ul>
      </div>
