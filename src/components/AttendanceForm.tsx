...
export default function AttendanceForm({ userId }: { userId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (file) {
      if (!ALLOWED_TYPES.includes(file.type)) {
        alert('File harus JPG/PNG/WEBP'); setLoading(false); return;
      }
      if (file.size > MAX_SIZE_MB * 1024 * 1024) {
        alert(`Maksimal ${MAX_SIZE_MB}MB`); setLoading(false); return;
      }
    ...
    if (insErr) alert(insErr.message); else { setName(''); setNote(''); setFile(null); }
    setLoading(false);
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-white rounded-2xl shadow">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700">Upload Foto (JPG/PNG/WEBP, ≤5MB)</label>
